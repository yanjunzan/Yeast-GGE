...
par(mar=c(4,4,2,1))
hist(num_loci_all,col="lightblue",xlab = "Number of detected \nenviroments (as epistatic loci)",main="",ylab = "Count",xpd=T,breaks=c(0:16))
mtext(text = "B",side = 3,line = -1,at = -5)
hist(as.numeric(ed_rank),col="lightblue",xpd=T,xlab = "Number of detected \n enviroments (pairwise interactions)",main="",ylab = "Count",breaks=c(0:9))
mtext(text = "D",side = 3,line = -1,at = 0)
